...
class FormAdmin(PageAdmin):
            ...
            dict(
                ...
                form_extra_fields={
                    "type": SelectField(
                        ...
                        choices=sorted(Field.type.info["choices"]),
                    )
                },
            ),
        )
    ...
    column_extra_row_actions.append(
    ...
    form_excluded_columns = PageAdmin.form_excluded_columns + [
        "author",
        "created",
        "updated",
        "entries",
    ...
    form_overrides.update({"submit_message": RichTextAreaField})
    ...
    def view_entries(self, pk):
        ...
        return self.render(
            "starlit_admin/form-entries.html",
            form=form,
            paginator=paginator,
...
def reg_form_admin(app, admin):
    ...
    if not app.config.get("FORM_UPLOADS_PATH", None):
        if uploads_path:
            ...
            raise StarlitConfigurationError(
                help_message="Neither the `FORM_UPLOADS_PATH` is defined, nor the `UPLOADS_PATH` is defined. Please add one of these to your application config, for the file admin to start."
            )
    admin.add_view(
        FormAdmin(
            Form, db.session, name=lazy_gettext("Form"), category=lazy_gettext("Pages")
        )
    )
