...
def process_events(event_name, session):
    for identity_set, modified in zip(("new", "dirty"), (False, True)):
        instances = (obj for obj in getattr(session, identity_set) if isinstance(obj, SQLAEvent))
...
def receive_after_flush_postexec(session, flush_context):
    call_method_in_all_bases((obj for obj in session), 'after_flush_postexec', session, True)
